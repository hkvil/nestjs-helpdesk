...
export class MasterDataService {
	...
	createChannel(dto: CreateChannelDto) {
		const c = this.channelRepo.create(dto as any);
	...
	async findChannel(id: number) {
		const channel = await this.channelRepo.findOne({ where: { id }, relations: ['categories'] });
	...
	async createCategory(dto: CreateCategoryDto) {
		...
		if (!channel) throw new NotFoundException(`Channel ${dto.channel_id} not found`);
		const cat = this.categoryRepo.create({ name: dto.name, channel } as any);
	...
	async findCategory(id: number) {
		const category = await this.categoryRepo.findOne({ where: { id }, relations: ['subCategories', 'channel'] });
	...
	async updateCategory(id: number, dto: UpdateCategoryDto) {
		...
		if ((dto as any).channel_id) {
			const channel = await this.channelRepo.findOneBy({ id: (dto as any).channel_id });
			if (!channel) throw new NotFoundException(`Channel ${(dto as any).channel_id} not found`);
			(category as any).channel = channel;
		}
	...
	async createSubCategory(dto: CreateSubCategoryDto) {
		...
		const sub = this.subCategoryRepo.create({ name: dto.name, category, priority: dto.priority } as any);
	...
	async findSubCategory(id: number) {
		const sub = await this.subCategoryRepo.findOne({ where: { id }, relations: ['category'] });
	...
	async updateSubCategory(id: number, dto: UpdateSubCategoryDto) {
		...
		if ((dto as any).category_id) {
			const category = await this.categoryRepo.findOneBy({ id: (dto as any).category_id });
			if (!category) throw new NotFoundException(`Category ${(dto as any).category_id} not found`);
			(sub as any).category = category;
		}
	...
	createPipeline(dto: CreatePipelineDto) {
		const p = this.pipelineRepo.create(dto as any);
