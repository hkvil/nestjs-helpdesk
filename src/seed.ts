...
// PicGroup and PicGroupMember types are currently not used in seed data; keep imports removed until needed
...
async function bootstrap() {
  ...
  // pic group repositories currently not used in seed but kept for future data
  ...
  const usersData = [
    {
      username: 'admin',
      email: 'admin@lppnpi.co.id',
      role: UserRole.SUPER_ADMIN,
    },
    { username: 'pic_sdm', email: 'pic.sdm@lppnpi.co.id', role: UserRole.PIC },
    {
      username: 'tech_it',
      email: 'tech.it@lppnpi.co.id',
      role: UserRole.TECHNICIAN,
    },
    {
      username: 'reporter1',
      email: 'reporter1@lppnpi.co.id',
    ...
    if (!existingTicket) {
      await ticketRepo.save(
        ticketRepo.create({
          ...
          pipeline:
            randomSub.category.channel.name === 'SDM'
              ? 'Tiket untuk SDM'
              : 'Tiket untuk IT',
          number: ticketNum,
