...
export class TicketService {
    ...
    async create(createTicketDto: CreateTicketDto) {
        // 1. Find Reporter
        const reporter = await this.reporterRepository.findOneBy({ id: createTicketDto.reporter_id });
        if (!reporter) {
            throw new NotFoundException(`Reporter with ID ${createTicketDto.reporter_id} not found`);
        }

        // 2. Determine SLA Policy
        let slaPolicy = await this.slaPolicyRepository.findOneBy({ pipeline: createTicketDto.pipeline });
        if (!slaPolicy) {
            slaPolicy = await this.slaPolicyRepository.findOneBy({ name: 'Standard SLA' });
        }
        ...
        const ticket = this.ticketRepository.create({
            ...
            due_date: dueDate,
            sla_policy: slaPolicy ? slaPolicy.name : null,
        ...
        stage.ticket = savedTicket as any;
        stage.stage = 'New';
        stage.percentage = 0;
        stage.pic = 'SYSTEM';
    ...
    async findOne(id: number) {
        const ticket = await this.ticketRepository.findOne({
            where: { id },
            relations: ['reporter', 'stages', 'assignees', 'attachments', 'associations'],
        });
    ...
    async update(id: number, updateTicketDto: UpdateTicketDto) {
        ...
        if (updateTicketDto.assignee_id) {
            ...
            if (!user) throw new NotFoundException(`User with ID ${updateTicketDto.assignee_id} not found`);

            ticket.assignee = user.username;

            // Create TicketAssignee
            const assignee = new TicketAssignee();
            assignee.ticket = ticket as any;
